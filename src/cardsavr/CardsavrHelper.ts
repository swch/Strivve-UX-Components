...
export class CardsavrHelper {
  ...
  private async saveSession(username: string, session: CardsavrSession) {
    ...
    if (localStorageAvailable()) {
      window.sessionStorage.setItem(
        `session_v2.3.1[${username}]`,
        session.getSerializedSessionData()
      );
    }
  }
  ...
  public getSession(username: string): CardsavrSession {
    ...
    throw new CardsavrSDKError(
      [],
      'Must login and create session before accessing session by username.'
    );
  }
  ...
  private async restoreSession(
    username: string,
    trace?: { [k: string]: unknown }
  ): Promise<CardsavrSession | null> {
    if (localStorageAvailable()) {
      const session_cache_data = <string>(
        window.sessionStorage.getItem(`session_v2.3.1[${username}]`)
      );
  ...
  public async createCard(
    create_card_config: createCardParams
  ): Promise<unknown> {
    const {
      card,
      agent_username,
    ...
    try {
      ...
      if (!cardholder_data_copy.name_on_card)
        card_data_copy.name_on_card = `${card_data_copy.first_name} ${card_data_copy.last_name}`;
      ...
      if (!card_data_copy.par) {
        card_data_copy.par = generateRandomPar(
          card_data_copy.pan,
          card_data_copy.expiration_month,
          card_data_copy.expiration_year,
          cardholder_data_copy.cuid
        );
      }
      ...
      const card_response = await agent_session.createCard(
        card_data_copy,
        safe_key,
        headers
      );
  ...
  public async placeCardOnSiteSingleCall(
    place_card_config: placeCardOnSiteParams
  ): Promise<unknown> {
    const {
      username,
      job_data,
    ...
    if (!cardholder) {
      throw new CardsavrSDKError(
        [],
        'Cannot create a job without a cardholder.'
      );
    }
    try {
      //set the missing settings for model
      ...
      if (card && !card.name_on_card)
        card.name_on_card = `${cardholder.first_name} ${cardholder.last_name}`;

      if (card && !card.par) {
        card.par = generateRandomPar(
          card.pan,
      ...
      const headers: { [k: string]: string | null } = {
        'x-cardsavr-hydration': JSON.stringify([
          'cardholder',
          'account',
          'card',
        ]),
      ...
      const response = await agent_session.createSingleSiteJob(
        job_data_copy,
        safe_key,
        headers
      );
  ...
  private handleError(err: any) {
    if (
      err instanceof CardsavrRestError ||
      (err.type && err.type === 'CardsavrRestError')
    ) {
      ...
      if (err.errors) {
        ...
        console.log(
          'Message returned from REST API: ' + err.response.body.message
        );
      }
    } else if (
      err instanceof CardsavrSDKError ||
      (err.type && err.type === 'CardsavrSDKError')
    ) {
  ...
  private async lookupMerchantSite(username: string, host: string) {
    ...
    const sites: CardsavrSessionResponse = await session.getMerchantSites({
      host: host,
    });
  ...
  public async placeCardOnSites(
    place_card_config: placeCardOnSitesParams
  ): Promise<any> {
    ...
    if (session) {
      try {
        jobs_data.forEach((job) => {
          job.account.cardholder_id =
            job.account.cardholder_id ?? job.cardholder_id;
          jobs.push(job);
        });
        return await session.createSingleSiteJobs(jobs, safe_key, {
          'x-cardsavr-hydration': JSON.stringify(['account']),
        });
      } catch (err) {
      ...
      throw new CardsavrSDKError(
        [],
        `No session established for: ${username}.  Need to call loginAndCreateSession?`
      );
    }
  }

  public async placeCardOnSite(
    place_card_config: placeCardOnSiteParams
  ): Promise<any> {
    ...
    if (session) {
      try {
        if (!account.site_id && account.site_hostname) {
          const site: any = await this.lookupMerchantSite(
            username,
            account.site_hostname
          );
          if (site) {
            account.site_id = site.id;
          }
        }
        return await this.placeCardOnSites({
          username,
          jobs_data: [job_data],
          safe_key,
        });
...
export class CardholderQuery {
  ...
  private async credsRequestHandler(message: jobMessage) {
    if (
      message.message?.status.startsWith('PENDING') &&
      !message.account_link
    ) {
      let tries = 2;
      while (tries-- >= 0) {
        const job = await this.session.getSingleSiteJobs(
          message.job_id,
          {},
          { 'x-cardsavr-hydration': JSON.stringify(['credential_requests']) }
        );
        if (job.body.credential_requests[0]) {
          if (message.message?.status.toLowerCase() !== 'pending') {
            this.event_emitter.emit(
              `${message.job_id}:${message.message?.status.toLowerCase()}`,
              job.body.credential_requests[0]
            );
          }
          this.event_emitter.emit(
            `${message.job_id}:pending`,
            job.body.credential_requests[0]
          );
          this.event_emitter.emit(
            `${message.job_id}:}`,
          ...
          this.event_emitter.emit(
            `${message.job_id}:${message.message?.status.toLowerCase()}`,
            {
              ...
              type: 'error',
              error_message: 'No credential requests for this job.',
            }
          );
        }
      }
    } else if (
      message.message?.termination_type &&
  ...
  public addListener(
    ...
    if (!this.creds_callbacks[job_id]) {
      this.creds_callbacks[job_id] = (message: jobMessage) =>
        this.credsRequestHandler(message);
  ...
  private runProbe() {
    ...
    this._user_probe = setInterval(async () => {
      try {
        ...
        if (tries++ > 10) {
          this.event_emitter.emitAll({
            job_id: -1,
            type: 'error',
            error_message: `CardSavr connection no longer available for cardholder_id: ${this.cardholder_id}.`,
          });
          this.removeAll();
        }
      }
...
export class EventEmitter {
  ...
  emitAll(data: jobMessage): void {
    Object.values(this.callbacks).map((cbs) =>
      cbs.forEach((cb) => {
        return cb(data);
      })
    );
