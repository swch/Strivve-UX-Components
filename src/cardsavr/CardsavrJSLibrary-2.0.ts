...
export class CardsavrSession {
  ...
  constructor(
    ...
    if (this._proxy && this._cardsavrCert) {
      throw new CardsavrSDKError([
        "Shouldn't be providing explicit certificate with proxy configuration.",
      ]);
    }
  ...
  setSessionHeaders = (headersObject: { [key: string]: string }): void => {
  ...
  sendRequest = async (
    ...
    if (requestBody) {
      requestBody = await CardsavrCrypto.Encryption.encryptRequest(
        sessionKey,
        requestBody
      );
      headers['content-type'] = 'application/json';
    }
    const authHeaders = await CardsavrCrypto.Signing.signRequest(
      path,
      this._appName,
      sessionKey,
      requestBody
    ...
    if (typeof window === 'undefined') {
      ...
      response = await window.fetch(
        new URL(path, this._baseUrl).toString(),
        config
      );
    }
    ...
    Object.entries(headers).forEach(
      ([key, val]) => (response_headers[key] = val)
    );

    CardsavrCrypto.Signing.verifySignature(
      response_headers,
      path,
      this._appName,
      [sessionKey],
  ...
  private _login = async (
    ...
    const client_public_key = await CardsavrCrypto.Keys.makeECDHPublicKey(
      key_pair
    );
    ...
    if (password) {
      const passwordKey = await CardsavrCrypto.Keys.generatePasswordKey(
        username,
        password
      );
      encrypted_login_body['password_proof'] =
        await CardsavrCrypto.Signing.signSaltWithPasswordKey(
          this._sessionData.sessionKey,
          passwordKey
        );
    } else {
      throw new CardsavrSDKError(
        [],
        'Must include password to initialize session.'
      );
    }
    ...
    this._sessionData.sessionKey = await CardsavrCrypto.Keys.makeECDHSecretKey(
      loginResponse.body.server_public_key,
      key_pair
    );
  ...
  setTrace = (username: string, trace?: { [k: string]: unknown }): void => {
    if (!trace) trace = {};
    if (trace instanceof Object && !trace.key) trace.key = username;
  ...
  updateAccount = async (
    ...
    if (envelope_id) {
      //envelope_id required to send a credential_re
  ...
  createFinancialInstitution = async (
    body: any,
    headersToAdd = {}
  ): Promise<any> => {
  ...
  updateFinancialInstitution = async (
    id: number,
    body: any,
    headersToAdd = {}
  ): Promise<any> => {
  ...
  deleteFinancialInstitution = async (
    id: number,
    headersToAdd = {}
  ): Promise<any> => {
  ...
  registerForJobStatusUpdates = async (
    jobId: number,
    headersToAdd = {}
  ): Promise<any> => {
    return await this.post(
      `/messages/place_card_on_single_site_jobs/${jobId}/broadcasts/registrations`,
      {
        job_id: jobId,
      },
      headersToAdd
  ...
  getCardholderMessages = async (
    cardholderId: number,
    cardsavrMessagingAccessKey?: string,
    headersToAdd = {}
  ): Promise<any> => {
    if (cardsavrMessagingAccessKey) {
      headersToAdd = Object.assign(
        {
          'x-cardsavr-cardsavr-messaging-access-key':
            cardsavrMessagingAccessKey,
        },
        headersToAdd
      );
    }
  ...
  getJobStatusUpdate = async (
    ...
    return await this.get(
      `/messages/place_card_on_single_site_jobs/${jobId}/broadcasts`,
      null,
      headersToAdd
    );
  };

  getJobInformationRequest = async (
    jobId: number,
    headersToAdd = {}
  ): Promise<any> => {
    return await this.get(
      `/messages/place_card_on_single_site_jobs/${jobId}/credential_requests`,
      null,
      headersToAdd
    );
  };

  getJobInformationResponse = async (
    jobId: number,
    headersToAdd = {}
  ): Promise<any> => {
    return await this.get(
      `/messages/place_card_on_single_site_jobs/${jobId}/credential_responses`,
      null,
      headersToAdd
    );
  };

  requestJobInformation = async (
    jobId: number,
    type: string,
    message: string,
    account_link: { [key: string]: string | boolean }[],
    headersToAdd = {}
  ): Promise<any> => {
    const body = {
      jobId,
  ...
  updatePassword = async (
    ...
    return await this.put(
      `/cardsavr_users/${id}/update_password`,
      null,
      body,
      headersToAdd
    );
  ...
  createCardholder = async (
    body: any,
    safeKey: string | null = null,
    financial_institution: string | null = null,
    headersToAdd = {}
  ): Promise<any> => {
    if (body && !body.cuid) {
