...
export class Encryption {
  ...
  static async encryptAES256(b64Key: string, clearText: string) {
    if (!browserCrypto) {
      ...
      const newBody = {
        encrypted_body:
          encryptedJSON.toString('base64') + '$' + IV.toString('base64'),
      };
      ...
      const newBody = {
        encrypted_body:
          WebConversions.arrayBufferToBase64(encryptedKey) +
          '$' +
          WebConversions.arrayBufferToBase64(iv),
      };
  ...
  static encryptSafeKeys(headers: any, b64Key: string) {
    const safe_key_headers = [
      'x-cardsavr-new-cardholder-safe-key',
      'x-cardsavr-cardholder-safe-key',
    ];
    safe_key_headers.forEach(async (safe_key_header) => {
      if (headers[safe_key_header]) {
        const encryptedObj = await this.encryptAES256(
          b64Key,
          headers[safe_key_header]
        );
  ...
  static async decryptResponse(key: string, body: any) {
    ...
    if (
      !body ||
      !Object.prototype.hasOwnProperty.call(body, 'encrypted_body')
    ) {
      return body;
    }
  ...
  static async decryptAES256(
    ...
    if (!browserCrypto) {
      ...
      const length =
        equalsIndex > -1 ? l - (b64cipherText.length - equalsIndex) : l;
...
export class Signing {
  static async sha256Hash(inputString: string) {
    if (!browserCrypto) {
      ...
      return await browserCrypto.subtle.digest(
        'SHA-256',
  ...
  static async verifySignature(
    ...
    if ('SWCH-HMAC-SHA256 Credentials=' + appName !== authorization) {
      throw new Error(
        'Authorization header not passed properly, should be: "SWCH-HMAC-SHA256 Credentials="' +
          appName
      );
    }
    ...
    if (
      !keys.some(
        async (key: string) =>
          signature === (await this.hmacSign(stringToSign, key))
      )
    ) {
  ...
  static async signSaltWithPasswordKey(
    sessionSalt: string,
    passwordKey: string
  ): Promise<string> {
  ...
  static async hmacSign(
    ...
    if (!browserCrypto) {
      ...
      const stringToSign: any = b64InputString
        ? WebConversions.base64ToArrayBuffer(inputString)
        : WebConversions.stringToArrayBuffer(inputString);
...
export class Keys {
  static async generatePasswordKey(
    ...
    return !browserCrypto
      ? (key as Buffer).toString('base64')
      : WebConversions.arrayBufferToBase64(key);
  }

  static async generateCardholderSafeKey(username: string): Promise<string> {
    if (!browserCrypto) {
      const saltedCardholderSafeKey = crypto.pbkdf2Sync(
  ...
  static async makeECDHPublicKey(myKeyPair: any) {
    if (!browserCrypto) {
      ...
      const publicKey = await browserCrypto.subtle.exportKey(
        'raw',
        myKeyPair.publicKey
      );
