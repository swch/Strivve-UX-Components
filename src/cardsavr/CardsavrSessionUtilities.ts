...
const _stringReplaceAll = function (
  string: any,
  find: string,
  replace: string
) {
  ...
  if (filter) {
    ...
    if (typeof filter === 'number') {
      ...
      validationErrors.push(
        'Valid ID or filter not found in provided path: ' +
          filter +
          ' type: ' +
          typeof filter
      );
...
export const createMetaKey = (
  ...
  return (
    first_name[0] + last_name[0] + postal_code.substring(0, 5) + pan.slice(-2)
  );
};
...
export const generateRandomPar = (
  pan: string,
  exp_month: string,
  exp_year: string,
  salt: string
  ...
  Object.entries(paramsArray)
    ...
    .forEach((param) =>
      validationErrors.push('Missing required parameter: ' + param[0])
    );

  if (
    (exp_month && exp_month.length != 2) ||
    isNaN(+exp_month) ||
    +exp_month > 12
  ) {
  ...
  const PARHash = crypto.pbkdf2Sync(
    panHash.toString('utf8'),
    salt_buffer,
    5000,
    20,
    'sha1'
...
export const getCardBrand = function (pan: string) {
  ...
  validationErrors.push(
    'Card number could not be matched to a recognized brand.'
  );
